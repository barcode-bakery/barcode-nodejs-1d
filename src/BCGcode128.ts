...
class BCGcode128 extends BCGBarcode1D {
    ...
    private static readonly KEYA_FNC2: number = 97;
    private static readonly KEYA_SHIFT: number = 98;
    private static readonly KEYA_CODEC: number = 99;
    private static readonly KEYA_CODEB: number = 100;
    private static readonly KEYA_FNC4: number = 101;
    private static readonly KEYA_FNC1: number = 102;

    private static readonly KEYB_FNC3: number = 96;
    private static readonly KEYB_FNC2: number = 97;
    private static readonly KEYB_SHIFT: number = 98;
    private static readonly KEYB_CODEC: number = 99;
    private static readonly KEYB_FNC4: number = 100;
    private static readonly KEYB_CODEA: number = 101;
    private static readonly KEYB_FNC1: number = 102;

    private static readonly KEYC_CODEB: number = 100;
    private static readonly KEYC_CODEA: number = 101;
    private static readonly KEYC_FNC1: number = 102;

    private static readonly KEY_STARTA: number = 103;
    private static readonly KEY_STARTB: number = 104;
    private static readonly KEY_STARTC: number = 105;

    private static readonly KEY_STOP: number = 106;
    ...
    constructor(start?: string | null) {
        ...
        /* CODE 128 B */
        this.keysB = ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~' + String.fromCharCode(127);
    ...
    parse(text: BCGDataInput<BCGcode128.Code> | BCGDataInput<BCGcode128.Code>[] | string) {
        ...
        if (seq !== '') {
            let bitstream = this.createBinaryStream(this.text, seq);
    ...
    getDimension(width: number, height: number): [number, number] {
        ...
        let textlength = this.data.length * 11;
        let endlength = 2; // + final bar
    ...
    protected processChecksum(): string | null {
        ...
        if (this.checksumValue !== null) {
            ...
            let keys = (this.lastTable === 'A') ? this.keysA : this.keysB;
    ...
    private getSequenceParsed(text: string, currentMode: string): string {
        ...
        if (this.tilde) {
            ...
            while ((pos = text.indexOf('~', previousPos)) !== -1) {
                let tildeData = this.extractTilde(text, pos);

                let simpleTilde = (tildeData === '~~');
    ...
    setParseB(text: string, currentMode: string | null): { value: string, currentMode: string } {
        let tmp = Utility.regexpQuote(this.keysB);
    ...
    private getSequence(text: string, startingText: string | null): string {
        ...
        let curLen = [startA, startB, startC];
        let curSeq = ['', '', ''];
        ...
        for (x = 0; x < xLen; x++) {
            ...
            if (this.tilde && input === '~') {
                let tildeData = this.extractTilde(text, x);
    ...
    private createBinaryStream(text: string, seq: string): [number[], string[]] {
        ...
        for (let i = 0, counter = 0; i < c; i++) {
            ...
            if (input === '.') {
                ...
                let shift = this.shift[currentEncoding][encoding];
                indcheck.push(shift);
                ...
                // We latch
                let latch = this.latch[currentEncoding][inputI];
                if (latch !== null) {
                    indcheck.push(latch);
     ...
     */
    private encodeChar(data: string[], encoding: number, seq: string, text: string, i: number, counter: number, indcheck: number[])
        : { data: string[], i: number, counter: number, indcheck: number[] } {
        if (seq[i + 1] !== undefined && seq[i + 1] === 'F') {
            // We have a flag !!
            if (text[counter + 1] === 'F') {
                let number = parseInt(text[counter + 2], 10);
                let fnc = this.fnc[encoding][number - 1];
                indcheck.push(fnc);
            ...
            if (encoding === 2) {
                ...
                let code = parseInt(text.substr(counter, 2), 10);
                indcheck.push(code);
                data.push(this.code[code]);
                counter++;
                i++;
            } else {
                let keys = (encoding === 0) ? this.keysA : this.keysB;
                let pos = keys.indexOf(text[counter]);
                indcheck.push(pos);
