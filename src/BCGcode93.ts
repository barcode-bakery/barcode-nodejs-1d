...
class BCGcode93 extends BCGBarcode1D {
    ...
    private static readonly EXTENDED_2: number = 44;
    private static readonly EXTENDED_3: number = 45;
    private static readonly EXTENDED_4: number = 46;
    ...
    parse(text: string): void {
        ...
        for (let i = 0; i < c; i++) {
            let pos = Utility.arraySearch(this.keys, this.text[i]);
            if (pos === -1) {
                // Search in extended?
                let extended = this.getExtendedVersion(this.text[i]);
                if (extended === null) {
                    throw new BCGParseException('Code93', 'The character \'' + this.text[i] + '\' is not allowed.');
                } else {
                    let extc = extended.length;
                    for (let j = 0; j < extc; j++) {
                        ...
                        if (v === '') {
                            ...
                            let pos2 = Utility.arraySearch(this.keys, v);
                            indcheck.push(pos2);
    ...
    getDimension(width: number, height: number): [number, number] {
        ...
        let startlength = 9;
        let textlength = 9 * this.data.length;
        let checksumlength = 2 * 9;
        let endlength = 9 + 1; // + final bar
    ...
    calculateChecksum() {
        ...
        let sequenceMultiplier = [20, 15];
        this.checksumValue = [];
        let indcheck = this.indcheck;
        for (let z = 0; z < 2; z++) {
            let checksum = 0;
    ...
    protected processChecksum(): string | null {
        ...
        if (this.checksumValue !== null) {
            let ret = '';
            let c = this.checksumValue.length;
            for (let i = 0; i < c; i++) {
