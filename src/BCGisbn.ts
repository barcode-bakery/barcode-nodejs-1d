...
class BCGisbn extends BCGean13 {
    ...
    protected addDefaultLabel(): void {
        if (this.isDefaultEanLabelEnabled()) {
            let Isbn = this.createISBNText();
            let font = this.font;

            let topLabel = new BCGLabel(Isbn, font, BCGLabel.Position.Top, BCGLabel.Alignment.Center);
    ...
    private createISBNText(): string {
        ...
        if (this.text !== '') {
            ...
            if (c === 12 || c === 13) {
                ...
                let checksum = this.processChecksum();
                isbn = 'ISBN ' + this.text.substr(0, 3) + '-' + this.text.substr(3, 9) + '-' + checksum;
                ...
                isbn = 'ISBN ' + this.text.substr(0, 9) + '-' + (checksum === 10 ? 'X' : checksum);
            }
        }
