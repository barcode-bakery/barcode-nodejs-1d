...
class BCGi25 extends BCGBarcode1D {
    ...
    draw(image: draw.Surface): void {
        ...
        for (let i = 0; i < c; i += 2) {
            let tempBar = '';
            let c2 = this.findCode(temptext[i])!.length; // !It has been validated
            for (let j = 0; j < c2; j++) {
                tempBar += this.findCode(temptext[i])!.substr(j, 1); // !It has been validated
                tempBar += this.findCode(temptext[i + 1])!.substr(j, 1); // !It has been validated
            }
    ...
    getDimension(width: number, height: number): [number, number] {
        let textlength = (3 + (this.ratio + 1) * 2) * this.text.length;
